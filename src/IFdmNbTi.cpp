...
double FDM::IFdmNbTi::GetCriticalTemp(double B)
{
  const int m = 5;
...
double FDM::IFdmNbTi::GetCSTemp(double T, double B, double I)
{
  double Tc  = GetCriticalTemp(B);
  double T0  = 4.5;
  double Ic  = GetCriticalCurrent(T) * 4.73 * 15. / 9.3;
  double Tcs = Tc + (Tc - T0) * (1 - I/Ic);
